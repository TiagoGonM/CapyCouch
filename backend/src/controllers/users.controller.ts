...
export const createUser: RequestHandler = async (req, res) => {
    ...
    if (userFound)
        return userFound && res.status(400).json({ message: "User already exists" });
...
export const updateUserById: RequestHandler = async (req, res) => {
    ...
    const { username, email, createdAt, status, id: mongoId, ...rest } = req.body as User;
...
export const deleteUser: RequestHandler = async (req, res) => {
    ...
    const user = await prisma.user.update({ where: { id }, data: { status: false } }) as User;
